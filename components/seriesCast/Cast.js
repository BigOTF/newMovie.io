...
const Cast = ({ params, seriesProfile }) => {
    ...
    function getKeyWords() {
        const fetchKeywords = async() => {
            try {
                const response = await fetch(`https://api.themoviedb.org/3/tv/${params.id}/keywords?api_key=${API_KEY}`);
                const data = await response.json();
                const keywordData = data.results;
    ...
    function castRoute() {
        return router.push(`/series/${params.id}/cast`)
    }
  ...
  return (
    <div className='px-10 py-4 flex gap-10'>
        ...
        <div className=' w-full flex flex-col gap-10'>
            ...
            {
                seriesProfile && <div className='flex flex-col gap-5'>

                    <div className='flex flex-col gap-5'>
                        <div className='flex flex-col'>
                            <p className='text-lg font-bold text-gray-950'>Status</p>
                            <p className='text-gray-700'>{seriesProfile.status}</p>
                        </div>
                        
                        <div className='flex flex-col'>
                            <p className='text-lg font-bold text-gray-950'>Network</p>
                            <ul className='flex flex-col gap-2'>{(seriesProfile.networks).map((network) => {
                                return (
                                    <li key={network.id}>
                                        <img src={`https://image.tmdb.org/t/p/w500${network.logo_path}`} className='w-[80px]'/>
                                    </li>
                                )
                            })}</ul>
                        </div>
                        ...
                        <div className='flex flex-col'>
                            <p className='text-lg font-bold text-gray-950'>Type</p>
                            <p className='text-gray-700'>{seriesProfile.type}</p>
                        </div>

                        <div className='flex flex-col'>
                            <p className='text-lg font-bold text-gray-950'>Original Language</p>
                            <p className='text-gray-700'>{seriesProfile.original_language}</p>
                        </div>
